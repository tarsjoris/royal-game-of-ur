...
class ExpectiMiniMaxPlayer(player: EPlayer, depth: Int) : AbstractExpectiMiniMaxPlayer(player, depth) {
    override fun heuristicOfPlayer(gameState: GameState, player: EPlayer) =
        Board.PATHS[player.index]
            .mapIndexed { i, loc -> if (gameState.getBox(loc) == player) i else 0 }
            .sum() + gameState.getFinishedCount(player) * Board.PATHS[player.index].size
}

class ExpectiMiniMaxPlayerWeights(player: EPlayer, depth: Int) : AbstractExpectiMiniMaxPlayer(player, depth) {
    override fun heuristicOfPlayer(gameState: GameState, player: EPlayer) =
...
abstract class AbstractExpectiMiniMaxPlayer(private val player: EPlayer, private val depth: Int) : IPlayer {
    override fun getName() =
        "MiniMax"

    override fun getNextMove(gameState: GameState, stepCount: Int) =
        getBestMoveAndScore(gameState, player, stepCount, depth).first

    private fun getBestMoveAndScore(
        gameState: GameState,
        player: EPlayer,
        stepCount: Int,
        depth: Int
    ...
    protected abstract fun heuristicOfPlayer(gameState: GameState, player: EPlayer): Int
